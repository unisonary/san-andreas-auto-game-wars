...
namespace GooglePlayGames.Native
{
	public class NativeTurnBasedMultiplayerClient : ITurnBasedMultiplayerClient
	{
		...
		{
			mTurnBasedManager.GetMatch(matchId, delegate(TurnBasedManager.TurnBasedMatchResponse response)
			{
				using (NativeTurnBasedMatch nativeTurnBasedMatch = response.Match())
				{
					...
					{
						callback(true, nativeTurnBasedMatch.AsTurnBasedMatch(mNativeClient.GetUserId()));
					}
				}
			});
		}
