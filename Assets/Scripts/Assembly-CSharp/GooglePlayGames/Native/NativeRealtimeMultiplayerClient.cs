	...
	public class NativeRealtimeMultiplayerClient : IRealTimeMultiplayerClient
	{
		...
		private abstract class MessagingEnabledState : State
		{
			...
			internal void UpdateCurrentRoom(NativeRealTimeRoom room)
			{
				...
				mNativeParticipants = Enumerable.ToDictionary(mRoom.Participants(), (GooglePlayGames.Native.PInvoke.MultiplayerParticipant p) => p.Id());
				mParticipants = Enumerable.ToDictionary(Enumerable.Select(mNativeParticipants.Values, (GooglePlayGames.Native.PInvoke.MultiplayerParticipant p) => p.AsParticipant()), (Participant p) => p.ParticipantId);
			}
			...
			{
				List<Participant> list = Enumerable.ToList(Enumerable.Where(mParticipants.Values, (Participant p) => p.IsConnectedToRoom));
				list.Sort();
				return list;
			}
		...
		private class ConnectingState : MessagingEnabledState
		{
			...
			internal override void HandleConnectedSetChanged(NativeRealTimeRoom room)
			{
				...
				IEnumerable<string> source = Enumerable.Except(mConnectedParticipants, hashSet);
				if (room.Status() == Types.RealTimeRoomStatus.DELETED)
				{
					Logger.e("Participants disconnected during room setup, failing. Participants were: " + string.Join(",", Enumerable.ToArray(source)));
					mSession.OnGameThreadListener().RoomConnected(false);
					mSession.EnterState(new ShutdownState(mSession));
					return;
				}
				IEnumerable<string> source2 = Enumerable.Except(hashSet, mConnectedParticipants);
				Logger.d("New participants connected: " + string.Join(",", Enumerable.ToArray(source2)));
				...
				{
					mPercentComplete += mPercentPerParticipant * (float)Enumerable.Count(source2);
		...
		private class ActiveState : MessagingEnabledState
		{
			...
			internal override void HandleConnectedSetChanged(NativeRealTimeRoom room)
			{
				...
				Dictionary<string, GooglePlayGames.Native.PInvoke.MultiplayerParticipant> dictionary = Enumerable.ToDictionary(room.Participants(), (GooglePlayGames.Native.PInvoke.MultiplayerParticipant p) => p.Id());
				foreach (string key in mNativeParticipants.Keys)
				{
				...
				mParticipants = Enumerable.ToDictionary(Enumerable.Select(mNativeParticipants.Values, (GooglePlayGames.Native.PInvoke.MultiplayerParticipant p) => p.AsParticipant()), (Participant p) => p.ParticipantId);
				Logger.d("Updated participant statuses: " + string.Join(",", Enumerable.ToArray(Enumerable.Select(mParticipants.Values, (Participant p) => p.ToString()))));
				...
				list = Enumerable.ToList(Enumerable.Where(list, (string peerId) => !peerId.Equals(selfId)));
				list2 = Enumerable.ToList(Enumerable.Where(list2, (string peerId) => !peerId.Equals(selfId)));
				if (list.Count > 0)
				{
					list.Sort();
					mSession.OnGameThreadListener().PeersConnected(Enumerable.ToArray(Enumerable.Where(list, (string peer) => !peer.Equals(selfId))));
				}
				if (list2.Count > 0)
				{
					list2.Sort();
					mSession.OnGameThreadListener().PeersDisconnected(Enumerable.ToArray(Enumerable.Where(list2, (string peer) => !peer.Equals(selfId))));
				}
			}
		...
		public void CreateWithInvitationScreen(uint minOpponents, uint maxOppponents, uint variant, RealTimeMultiplayerListener listener)
		{
			lock (mSessionLock)
			{
				...
				mRealtimeManager.ShowPlayerSelectUI(minOpponents, maxOppponents, true, delegate(PlayerSelectUIResponse response)
				{
					...
					mCurrentSession.MinPlayersToStart = (uint)((int)response.MinimumAutomatchingPlayers() + Enumerable.Count(response) + 1);
					using (RealtimeRoomConfigBuilder realtimeRoomConfigBuilder = RealtimeRoomConfigBuilder.Create())
					{
